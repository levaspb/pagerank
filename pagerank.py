...
def transition_model(corpus, page, damping_factor):
    ...
    """
    transition = {item: 0 for item in corpus}
    for item in corpus:
        if item in corpus[page]:
            transition[item] += 1 / len(corpus[page]) * damping_factor
        transition[item] += 1 / len(corpus) * (1 - damping_factor)
